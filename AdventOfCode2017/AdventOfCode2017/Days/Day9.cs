...
@"{{<a!>},{<a!>},{<a!>},{<ab>}}";
        ...
        private const char GARBAGE_START = '<';
        private const char GARBAGE_END = '>';
        private const char IGNORE_FLAG = '!';
        #endregion

        public void Part1()
        {
            Console.WriteLine("Stream Processing P.1");

            // Start at 1 since we are guarenteed an outside containing group
            int scoreValue = 1;
            int score = scoreValue;
            bool inGarbage = false;

            // Extract inside string (remove containing group)
            string input = _input.Substring(1, _input.Length - 2);

            using (var enumerator = input.GetEnumerator())
            {
                while (enumerator.MoveNext())
                {                    
                    // Skip the IGNORE_FLAG and the following character
                    if (enumerator.Current == IGNORE_FLAG)
                    {
                        enumerator.MoveNext();
                        continue;
                    }

                    // If we are in a garbage block, keep skipping til we reach the end
                    if (inGarbage && enumerator.Current != GARBAGE_END)
                        continue;
        ...
        public void Part2()
        {
            Console.WriteLine("Stream Processing P.2");
            ...
            using (var enumerator = input.GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    ...
                    inGarbage = enumerator.Current == GARBAGE_START;
                }
            }

            Console.WriteLine($"Non-Canceled Characters in Garbage: {count}");
        }
    }
}
