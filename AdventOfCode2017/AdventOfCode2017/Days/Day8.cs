...
c inc -20 if c == 10";
        ...
        public void Part1()
        {
            Console.WriteLine("I Heard You Like Registers P.1");
            
            foreach (string line in _input.Split('\n').Select(l => l.Trim()))
            {
                var instruction = new Instruction()
                {
                    Register = GetRegisterName(line),
                    Operation = GetOperation(line),
                    Condition = GetCondition(line)
                };

                if (!_registers.ContainsKey(instruction.Register))
                    _registers.Add(instruction.Register, 0);
                
                string conditionRegisterName = GetConditionRegisterName(instruction.Condition);
                if (!_registers.ContainsKey(conditionRegisterName))
                    _registers.Add(conditionRegisterName, 0);
                
                if (instruction.MeetsCondition(_registers[conditionRegisterName]))
                    _registers[instruction.Register] += instruction.GetOperationAmount();
            }

            Console.WriteLine($"Largest Register Value: {_registers.Max(r => r.Value)}");
        }

        public void Part2()
        {
            Console.WriteLine("I Heard You Like Registers P.2");
            int largestValueEver = int.MinValue;
            ...
            foreach (string line in _input.Split('\n').Select(l => l.Trim()))
            {
                ...
                if (_registers[instruction.Register] > largestValueEver)
                    largestValueEver = _registers[instruction.Register];
            }

            Console.WriteLine($"Largest Register Value Ever: {largestValueEver}");
        }

        private string GetRegisterName(string line)
        {
            Regex regex = new Regex(@"^[a-zA-Z]+");
            if (!regex.IsMatch(line))
                throw new ApplicationException($"Failed to find the Register Name in line \"{line}\"");

            return regex.Match(line).Value.Trim();
        }

        private string GetOperation(string line)
        {
            Regex regex = new Regex(@"(inc|dec){1}\s+(-)?[0-9]+");
            if (!regex.IsMatch(line))
                throw new ApplicationException($"Failed to find the Register Operation in line \"{line}\"");
        ...
        {
            Regex regex = new Regex(@"[a-zA-Z]+\s+(<|<=|==|!=|>=|>){1}\s+(-)?[0-9]+$");
            if (!regex.IsMatch(line))
                throw new ApplicationException($"Failed to find the Register Condition in line \"{line}\"");

            return regex.Match(line).Value.Trim();
        }
        
        private string GetConditionRegisterName(string condition)
        {
            Regex regex = new Regex(@"^[a-zA-Z]+");
            if (!regex.IsMatch(condition))
                throw new ApplicationException($"Failed to find the Register Condition Name in condition \"{condition}\"");

            return regex.Match(condition).Value.Trim();
        }
    }
    ...
    class Instruction
    {
        ...
        public int GetOperationAmount()
        {
            ...
            int amount = int.Parse(regex.Match(Operation).Value);

            return Operation.Contains("dec") ? 
                amount * -1 : amount;
        }
        
        public bool MeetsCondition(int valueToCheck)
        {
            Regex regex = new Regex("(-)?[0-9]+");
            int value = int.Parse(regex.Match(Condition).Value);
            ...
            throw new ApplicationException($"Failed to perform condition \"{Condition}\"");
        }
    }
}
